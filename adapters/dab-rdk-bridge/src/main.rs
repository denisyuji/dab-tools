...
pub fn main() {
    ...
    //     if let Ok(Incoming(packet)) = notification{
    //         if let Packet::Publish(p,prop_opt) = *packet{
    // 
    //             let result: String;
    //             let function_topic = std::string::String::from_utf8((&p.topic).to_vec()).unwrap();
    // 
    //             match handlers.get_mut(&function_topic) {
    //                 Some(callback) => {
    //                     println!("OK: {}",function_topic);
    ...
    //             // Set topic properties
    //             let mut msg_prop = Properties::new();
    //             if let Err(e) = msg_prop.push_val(PropertyCode::ResponseTopic,"test"){
    //                 println!("Error setting Msg Properties: {:?}", e);
    //                 process::exit(1);
    //             }
    // 
    ...
    //                         .finalize();
    // 
    //             println!("Publishing messages on the {:?} topic", TOPICS[0]);
    ...
    //             //     let response_topic = prop.response_topic.unwrap();
    //             //     if let Err(e) = client.publish(&response_topic,QoS::AtMostOnce, false, result) {
    //             //         println!("ERROR: unable to publish response on topic: {}", e);
    //             //     };
    //             // }
    //         }
    //     }
